...
@EFragment(R.layout.task_10)
public class Task_10Fragment extends Fragment {

    private static final String EMPTY = "Не выбрано";
    private List<String> list_1, list_2, list_3, list_4, list_5;

    @ViewById
    Spinner input_1, input_2, input_3, input_4, input_5;

    @AfterViews
    void ready(){
        getActivity().setTitle(R.string.task_10);
        getSpinnerData();
    }
    ...
    void getSpinnerData(){
        ...
        list_4.add(0, EMPTY);
        list_5 = new ArrayList<>(3);
        list_5.add(EMPTY);
        list_5.add("Более пяти специалистов");
        list_5.add("Ни одного специалиста");
        setSpinnerData();
    }
    ...
    private List<String> getData(){
        String  data_1, data_2, data_3, data_4, data_5 = null;
        ...
        if (!input_5.getSelectedItem().toString().equalsIgnoreCase(EMPTY)) {
            if (input_5.getSelectedItem().toString().equals("Более пяти специалистов"))
                data_5 = "HAVING COUNT(military._id_mlt) >= 5";
            if (input_5.getSelectedItem().toString().equals("Ни одного специалиста"))
                data_5 = "HAVING COUNT(military._id_mlt) = 0";
        }
        else data_5 = "";
        if (!data_1.isEmpty()) return DataManager.getDataT10(getContext(), data_1, data_2, data_3, data_4, data_5);
        return null;
    }
